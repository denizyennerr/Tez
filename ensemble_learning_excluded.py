...
# Academic Integrity: Split data for threshold/weight optimization vs testing
...
# ---> NEW: List the 6 weak subject IDs here to exclude them from the analysis
EXCLUDED_SUBJECTS = [6,9,10,12,13,14,16]

# ---> NEW: Changed output directory so it doesn't overwrite your full dataset results
OUTPUT_DIR = os.path.join('saved_outputs_play', 'ensemble_results_filtered')
...
def main() -> None:
    ...
    # ---> NEW: Filter out the excluded subjects immediately
    all_subjects = np.unique(ref_data['s'])

    # FIX: Extract only the numerical digits from strings like 'chb01' before checking
    subjects = np.array([
        s for s in all_subjects
        if int(''.join(filter(str.isdigit, str(s)))) not in EXCLUDED_SUBJECTS
    ])

    logging.info(
        f"Loaded {len(all_subjects)} subjects. Excluded {len(all_subjects) - len(subjects)}. Proceeding with {len(subjects)} subjects."
    )
    ...
    if not per_subject_df.empty:
        ...
        logging.info('=== FINAL FILTERED TEST SET METRICS ===')

...
def plot_testing_evaluation(y_test, y_pred_prob, y_pred_class, subject, save_path):
    ...
    Four-panel evaluation figure:
      1. Confusion Matrix          (operating-point at fixed threshold)
...
# --- FIX: Swap axes if the sample dimension is not on the first axis ---
if X.shape[0] != y.shape[0]:
    print(f"⚠️ Swapping axes for X. Original shape: {X.shape}")
    X = np.swapaxes(X, 0, 1)
...
# --- ADDED: Arrays to collect all predictions and labels across subjects ---
all_y_test = []
all_y_pred_prob = []
all_y_pred_class = []
# -------------------------------------------------------------------------
...
for train_idx, test_idx in logo.split(X, y, groups=groups):
    ...
    # --- ADDED: Store predictions and ground truths for the aggregate evaluation ---
    all_y_test.append(y_test)
    all_y_pred_prob.append(y_pred_prob)
    all_y_pred_class.append(y_pred_class)
    # -------------------------------------------------------------------------------
...
if all_reports:
    ...
    print(f"\n✅  Primary aggregate bar plot saved to: {primary_plot_path}")

    plot_secondary_metrics(summary_df, secondary_plot_path, DECISION_THRESHOLD)
    print(f"✅  Secondary aggregate bar plot saved to: {secondary_plot_path}")
    ...
    # --- ADDED: Generate and save the 4-panel Aggregate Testing Evaluation Plot ---
    print("\n📈  Generating Aggregate Testing Evaluation Plot...")
    y_test_agg = np.concatenate(all_y_test)
    y_pred_prob_agg = np.concatenate(all_y_pred_prob)
    y_pred_class_agg = np.concatenate(all_y_pred_class)

    agg_eval_path = os.path.join(plots_dir, f"aggregate_testing_evaluation_{suffix}.png")
    plot_testing_evaluation(
        y_test_agg,
        y_pred_prob_agg,
        y_pred_class_agg,
    ...
    print(f"✅  Aggregate evaluation 4-panel plot saved to: {agg_eval_path}")
    # ------------------------------------------------------------------------------
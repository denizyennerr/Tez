...
# Lower threshold = Higher Sensitivity (more easily predicts 'Seizure'), but Lower Specificity
...
def plot_testing_evaluation(y_test, y_pred_prob, current_subject, save_path, threshold=0.5):
    """Plots Confusion Matrix and Precision-Recall Curve separately, respecting custom thresholds."""
    y_pred = (y_pred_prob > threshold).astype(int)

    fig, axs = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(f'Testing Evaluation | Subject: {current_subject} (Threshold: {threshold})', fontsize=16,
                 fontweight='bold')
...
for train_idx, test_idx in logo.split(X, y, groups=groups):
    ...
    # Determine threshold for current subject
    current_threshold = SUBJECT_THRESHOLDS.get(current_test_subject, DEFAULT_THRESHOLD)
    print(f"\n🧪 Evaluating Subject: {current_test_subject} (Threshold: {current_threshold})")
    ...
    y_pred_class = (y_pred_prob > current_threshold).astype(int)
    ...
    all_reports.append({
        ...
        'threshold': current_threshold,  # Log the threshold used
    ...
    plot_testing_evaluation(y_test, y_pred_prob, current_test_subject, test_save_filepath, threshold=current_threshold)

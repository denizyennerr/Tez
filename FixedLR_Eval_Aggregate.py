...
def plot_aggregate_testing_evaluation(y_test_all, y_pred_prob_all, y_pred_class_all, save_path):
    auroc = roc_auc_score(y_test_all, y_pred_prob_all)
    auprc = average_precision_score(y_test_all, y_pred_prob_all)
    fpr, tpr, _ = roc_curve(y_test_all, y_pred_prob_all)
    prec, rec, _ = precision_recall_curve(y_test_all, y_pred_prob_all)
    ...
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.plot(fpr, tpr, color="steelblue", lw=2.5, label=f"Global AUROC = {auroc:.4f}")
    ...
    ax4.set_title("Global Predicted Probability Distribution", fontsize=14)
...
if all_reports:
    ...
    # 🌟 NEW: Generate and save the Global 4-Panel Plot
    print("\n📈  Generating Global Aggregate Evaluation Plot...")
    global_y_test = np.concatenate(global_y_test_list)
    global_y_pred_prob = np.concatenate(global_y_pred_prob_list)
    global_y_pred_class = np.concatenate(global_y_pred_class_list)

    aggregate_eval_path = os.path.join(plots_dir, f"aggregate_testing_evaluation_{suffix}.png")
    plot_aggregate_testing_evaluation(global_y_test, global_y_pred_prob, global_y_pred_class, aggregate_eval_path)

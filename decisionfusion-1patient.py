...
# =============================================================================
# LOSO ENSEMBLE EVALUATION - REFACTORED FOR PATIENT 1 ONLY
# =============================================================================
all_reports = []

# Get the split for the first subject only (Patient 1 / chb01)
splits = list(logo.split(datasets[BASE_DS_NAME]['X'], y_base, groups=groups_base))
train_idx_base, test_idx_base = splits[0]

current_test_subject = groups_base[test_idx_base][0]
print(f"\n🧪 Evaluating Ensemble for Subject: {current_test_subject}")
...
# Loop through models defined in ENSEMBLE_CONFIG
for ds_path, cfg in ENSEMBLE_CONFIG.items():
    if cfg['weight'] == 0.0: continue

    model_path = os.path.join(cfg['models_dir'], f'best_model_subject_{current_test_subject}.keras')

    # Load data for this specific model scale
    ...
    # ALIGNMENT: Convert N-second probs to 1-second resolution
...
# 🔥 FIX: CALCULATE METRICS TO AVOID NAMEERROR
cm = confusion_matrix(y_test_actual, ensemble_class)
tn, fp, fn, tp = cm.ravel()  # This defines the variables you were missing
...
accuracy = (tp + tn) / len(y_test_actual)
seizure_f1 = np.max(f1_scores)

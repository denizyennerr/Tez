...
TIMESTAMP = "saved_outputs_play/20260322-020024_10.0s"
...
def plot_secondary_metrics(summary_df, save_path, DECISION_THRESHOLD=0.3):
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Score")
    ax.set_title("Secondary Performance Metrics")
...
for train_idx, test_idx in logo.split(X, y, groups=groups):
    ...
    # -------------------------------------------------------------
    # [CRITICAL FIX] Test Seti Sınıf Kontrolü
    # -------------------------------------------------------------
    # Eğer test setinde sadece tek bir sınıf varsa (hiç nöbet yoksa)
    # AUROC ve AUPRC gibi metrikler hesaplanırken script hata verip çöker.
    if len(np.unique(y_test)) < 2:
        print(f"⚠️ DİKKAT: Fold {current_test_subject} test setinde hiç nöbet verisi yok (veya hep nöbet)!")
        print("AUROC/AUPRC hesaplanamayacağı için bu fold değerlendirmesi atlanıyor...")
        continue

    # ── Load model and history ────────────────────────────────────────────────
    # [CRITICAL FIX] model yüklerken custom metric hatalarını önlemek için compile=False
    ...
    # ── Test-set predictions (Dengesiz/Doğal Veri) ────────────────────────────
    print(f"  -> Testing on: {len(y_test[y_test == 1])} Seizures, {len(y_test[y_test == 0])} Normal windows")

...
output_csv_name = "all_models_performance_comparison_1s_stride_corrected.csv"
...
# 2. CORE FUNCTIONS
# =============================================================================
def window_level_align(coarse_probs: np.ndarray, target_length: int, window_name: str) -> np.ndarray:
    """
    Robust alignment of probabilities.
    If the lengths match, it means a 1s stride was used (Solution B).
    If they don't, it performs step-upsampling (Solution A) rather than simple interpolation.
    """
    n_coarse = len(coarse_probs)

    # Solution B: Data was generated with 1s stride, perfect temporal alignment
    if n_coarse == target_length:
        print(f"    ✓ Perfect alignment detected for {window_name} (1s Stride confirmed).")
        return coarse_probs.copy()

    # Solution A: Upsampling fallback
    print(f"    ⚠️ Resolution mismatch for {window_name} ({n_coarse} vs {target_length}). Upsampling...")

    # Calculate how many 1s blocks fit into this window (e.g., 2s window = repeat 2 times)
    repeat_factor = max(1, target_length // n_coarse)
    upsampled_probs = np.repeat(coarse_probs, repeat_factor)

    # Trim or pad to exactly match the target length (handling edge cases at the end of the recording)
    if len(upsampled_probs) > target_length:
        upsampled_probs = upsampled_probs[:target_length]
    elif len(upsampled_probs) < target_length:
        padding = np.full(target_length - len(upsampled_probs), upsampled_probs[-1])
        upsampled_probs = np.concatenate([upsampled_probs, padding])
...
for subject in subjects:
    ...
    for w_name, info in datasets_info.items():
        ...
        if os.path.exists(model_path):
            ...
            # Using the robust alignment function
            aligned_probs = window_level_align(probs, target_len, w_name)
